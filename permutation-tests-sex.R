...
### Date: 4/22/2025
### Description: Permutation testing to test for significant differences in 
### average epigenetic age deviation between sex
...
info_vars <- c('subjectid', 'Timepoint', 'Tissue', 'sex_.0.F.', 'Sample_Name')
...
combined <- ead %>%
  ...
  mutate(sex = case_when(sex_.0.F. == 0 ~ 'female',
                         sex_.0.F. == 1 ~ 'male'))

# calculate difference in EAD and EAD trajectory
ead_diff <- combined %>%
  pivot_wider(id_cols = c(subjectid, Tissue, sex),
...
permutation_test <- function(df, # (dataframe) data
  ...
  for (i in 1:p_n){
    ...
    # calculate observed test-stat: difference in mean EAD between sex
    perm_test_stats[i] <- mean(perm_samples[df$sex == 'male', i]) -
      mean(perm_samples[df$sex == 'female', i])
    # mean EAD in male participants - mean EAD in female participants
  }
  ...
  # get observed mean EAD in female participants 
  mean_female <- filter(df, sex == 'female') %>%
    pull(response) %>%
    mean()
  
  # get observed mean EAD in male participants
  mean_male <- filter(df, sex == 'male') %>%
    pull(response) %>%
    mean()
  
  # calculate observed test statistic 
  obs_test_stat <- mean_male - mean_female
