...
################################################################################
# blood samples
blood_pedbe <- lmer(ped_be ~ pearls + age + sex  + income_FPL_100 + (1 | pearls_id),
                    data = filter(clean, tissue == 'blood'))

blood_horvath2 <- lmer(horvath2 ~ pearls + age + sex + income_FPL_100 + (1 | pearls_id),
                       data = filter(clean, tissue == 'blood'))

blood_horvath1 <- lmer(horvath_age ~ pearls + age + sex + income_FPL_100 + (1 | pearls_id),
                       data = filter(clean, tissue == 'blood'))

# buccal samples
buccal_pedbe <- lmer(ped_be ~ pearls + age + sex + income_FPL_100 + (1 | pearls_id),
                     data = filter(clean, tissue == 'buccal'))

buccal_horvath2 <- lmer(horvath2 ~ pearls + age + sex + income_FPL_100 + (1 | pearls_id),
                        data = filter(clean, tissue == 'buccal'))

buccal_horvath1 <- lmer(horvath_age ~ pearls + age + sex + income_FPL_100 + (1 | pearls_id),
                        data = filter(clean, tissue == 'buccal'))
...
# output
################################################################################
write.csv(results, 'data-processed/mm-results.csv', row.names = FALSE)