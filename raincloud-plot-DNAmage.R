...
### Date: 3/31/2026
### Description: Visualize distribution of epigenetic age 
...
dnam_age <- read.csv('data-processed/methylation-predictions.csv')
...
clean_data <- dnam_age %>%
  ...
  select(SampleID, Horvath2, PedBE) %>%
...
raincloud <- function(df, # data of interest
                      ...
                      clock # clock used to generate DNAm age
){
  
  df %>%
    filter(Tissue == tissue) %>% # plot data from specified tissue type only
    ggplot(aes(x = Timepoint, # show DNAm age distribution at each timepoint
               y = !!sym(clock) # plot DNAm age from specified clock
    )
    ) +
    ...
    labs(
      title = "Epigenetic age across time",
      y = "Years",
      x = "Timepoint",
      color = "PEARLS"
    ) +
...
sb_blood <- raincloud(clean_data, 'Blood', 'Horvath2') +
  labs(subtitle = 'Skin & Blood clock in blood samples')

# Skin & Blood clock in buccal samples
sb_buccal <- raincloud(clean_data, 'Buccal', 'Horvath2') +
  labs(subtitle = 'Skin & Blood clock in buccal samples')

# PedBE clock in blood samples
pbe_blood <- raincloud(clean_data, 'Blood', 'PedBE') +
  labs(subtitle = 'PedBE clock in blood samples')

# PedBE clock in buccal samples
pbe_buccal <- raincloud(clean_data, 'Buccal', 'PedBE') +
  labs(subtitle = 'PedBE clock in buccal samples')

# output 
################################################################################
# save plots as PNG in figures folder
ggsave('figures/raincloud-dnamage-horvath2-blood.png', 
       plot = sb_blood,
       width = 5.97,
       height = 4.5,
       units = 'in')

ggsave('figures/raincloud-dnamage-horvath2-buccal.png', 
       plot = sb_buccal,
       width = 5.97,
       height = 4.5,
       units = 'in')

ggsave('figures/raincloud-dnamage-pedbe-blood.png', 
       plot = pbe_blood,
       width = 5.97,
       height = 4.5,
       units = 'in')

ggsave('figures/raincloud-dnamage-pedbe-buccal.png', 
       plot = pbe_buccal,
       width = 5.97,
       height = 4.5,
       units = 'in')